...
class perf:
    ...
    def get_stock_volume(self):
        """ Return Pandas DF of all stock volumes """
        lot = []
        for i in range(len(self.stocks)):
            t = pd.read_sql(f''' select date(Date) as Date, Volume as "{self.stocks[i].upper()}" from {self.stocks[i]} order by Date asc ''', self.daily_db, index_col='Date')
            t.index = pd.to_datetime(t.index)
            if len(t) > 30:
                lot.append(t)
        return pd.concat(lot, axis = 1)

    def get_returns(self, sp = None):
        """ 
