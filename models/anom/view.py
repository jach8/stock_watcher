...
from model import StackedAnomalyModel
...
class EnhancedViewer(StackedAnomalyModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.price_data = price_data  # DataFrame containing ohlcv
        # Custom color maps
    ...
    def plot_individual_models(self, data: Optional[pd.DataFrame] = None) -> None:
        ...
        for i, (name, model) in enumerate(self.base_models.items()):
            ...
            close = self.price_data.loc[self.testing_dates, 'close'].values
            # Plot time series with predictions
            scatter = ax.scatter(self.testing_dates, close, 
                               c=preds, cmap=self.cmap, 
                               alpha=0.6, s=50)
            ax.plot(self.testing_dates, close, color='black', alpha=0.2)
            
            # Add confidence band if scores available
            if scores is not None:
                scores = StandardScaler().fit_transform(scores.reshape(-1, 1)).ravel()
                ax.fill_between(self.testing_dates, close, 
                              close + scores, 
        ...
        # Add meta-learner plot
        ax = plt.subplot(gs[-1, -1])
        meta_preds = self.predict(data).values
        close = self.price_data.loc[self.testing_dates, 'close'].values
        scatter = ax.scatter(self.testing_dates, close,
                                 c=meta_preds, cmap=self.cmap, 
                                 alpha=0.6, s=50)
        ax.plot(self.testing_dates, close, color='black', alpha=0.2)
        ax.set_title('Meta-Learner Predictions')
        ax.set_xlabel('Date')
        ax.set_ylabel('Price')
        ax.legend(*scatter.legend_elements(), title="Predictions", loc='upper left')
    ...
    def plot_confidence_distributions(self) -> None:
        ...
        for ax, (name, scores) in zip(axes, scores_dict.items()):
            ...
            normal_scores = [scores[x] for x, i in enumerate(self.ytest) if i == 0]
            anomaly_scores = [scores[x] for x, i in enumerate(self.ytest) if i == 1]
    ...
    def plot_temporal_agreement(self, window: int = 30) -> None:
        ...
        for name, model in self.base_models.items():
            preds = model.predict(self.xtest)
            all_preds.append(pd.Series(preds, index=self.testing_dates, name=name))
            
        meta_preds = self.predict(self.xtest)
        all_preds.append(pd.Series(meta_preds.values.ravel(), index=self.testing_dates, name='Meta-Learner'))
        ...
        ax1.plot(self.testing_dates, self.price_data['close'].loc[self.testing_dates], color='black', alpha=0.5)
...
if __name__ == "__main__":
    ...
    x, y = d._returnxy('spy')
    price_data = d.price_data('spy')
    print(f'X:{x.shape}\n', x, '\n\n')
    print(f'Y: {y.shape}', y)
    
    # Create and fit model
    print("\nInitializing and fitting model...")
    model = EnhancedViewer(x, y, price_data=price_data,  verbose=True, stock='spy')
    model.fit(x=x, y=y, t=100, start_date = '2020-01-01', end_date = '2025-03-01')
