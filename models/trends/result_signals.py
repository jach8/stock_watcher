...
Signal Generation for Price, Volume, and Open Interest Trends
- We dont need any numerical signals, just categorical ones.
...
class WorksheetEntry:
    ...
    returns_category: Optional[str] = None  # Optional field for returns category
...
class VolumeOpenInterestWorksheet:
    ...
    def __init__(
        ...
        todays_return = returns_data.tail(1)
        ...
        for metric, log in classification_logs.items():
            if metric == 'stock_volume':
                ...
                returns_category = 'Up' if todays_return.iloc[0] > 0 else 'Down'
        ...
        for i, (date, price) in enumerate(price_data.items()):
            self.entries.append(WorksheetEntry(
                date=date,
                price=price,
                returns=todays_return.iloc[i] if not pd.isna(todays_return.iloc[i]) else 0.0,
                ...
                returns_category=returns_category if 'returns' in classification_logs else None
            ))
    ...
    def to_df(self) -> pd.DataFrame:
        ...
        for entry in self.entries:
            data.append({
                ...
                'Returns_Category': entry.returns_category,  # New field
                'Volume': entry.volume,
